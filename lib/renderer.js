...
class ColumnRenderer {
    ...
    toDot() {
        ...
        const coloredName = rawColor && rawColor.trim()
...
class CompositeKeyRowRenderer {
    ...
    toDot() {
        return `<TR><TD PORT="${this.port}" BGCOLOR="#ffffff"><FONT COLOR="#1d71b8"><I>    ${this.columns.join(", ")}    </I></FONT></TD></TR>`;
    }
}
class TableRenderer {
    constructor(table) {
    ...
    displayName() {
        return (0, common_1.fullName)(this.table.actual);
    }
    toDot() {
...
class EnumRenderer {
    ...
    name() {
        return (0, common_1.fullName)(this.enumType.actual);
    }
    selfRef() {
        return `"${this.name()}":f0`;
    }
